...
router.get('/dados', auth, (req, res) => {
    ...
    // Normaliza: PDF import salva { quantidade, envio }, código antigo salva números direto
...
router.post('/upload_pdf', auth, upload.single('pdf'), async (req, res) => {
    ...
    try {
        ...
        // entra calado no envio — foi o que aconteceu no #74910211, que registrou 844
        // unidades num PDF de 98 porque a instrucao "260 cm." virou quantidade.
        const somaLida = Object.values(skus).reduce((a, b) => a + b, 0);
        if (totalUnidades > 0 && somaLida !== totalUnidades) {
            return res.status(400).json({
                erro: `Leitura inconsistente: o PDF declara ${totalUnidades} unidades, mas os ` +
                      `itens somam ${somaLida}. Envio nao cadastrado. Confira o PDF ou cadastre a mao.`,
                declarado: totalUnidades,
                somado: somaLida,
                skus,
            });
        }
...
// Contexto de mercado (Nubimetrics) de UM produto parado. So e chamado quando a
// gaveta daquele produto abre: a cota do Nubimetrics e por token e compartilhada
// com os outros projetos, entao nada de consultar a lista inteira de uma vez.
router.get('/parados/mercado', auth, async (req, res) => {
    try {
        ...
        if (!p) return res.status(404).json({ erro: 'Produto nao esta na lista de parados.' });
...
router.post('/gerar_planilha', auth, (req, res) => {
    try {
        const XLSX = require('xlsx');
        const itens = req.body; // [{ item_id, sku, variation_id, qtdFull }]
...
router.post('/switch', auth, (req, res) => {
    ...
    // Resolve o caminho do exe. O nome exibido ("Flavia Stock") nem sempre é o
    // productName da instalação ("Dashboard ML"), então além dos caminhos exatos
    // procuramos uma pasta instalada que compartilhe uma palavra do nome.
    const exePath = (() => {
