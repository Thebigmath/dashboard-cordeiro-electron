...
// traz a janela de volta em vez de abrir um segundo servidor na porta 3002.
...
app.setAppUserModelId('com.thebigmath.dashboard-cordeiro');
...
require('./lib/notificar').usarNotificador((titulo, corpo, rota, opcoes = {}) => {
    ...
    n.on('click', () => {
        if (!mainWindow) return;
        mostrarJanela();
        if (rota) mainWindow.loadURL('http://localhost:3002' + rota);
    });
    n.show();
});
...
autoUpdater.on('update-downloaded', (info) => {
    ...
    if (Notification.isSupported()) {
        const n = new Notification({ title: `Cordeiro Car: atualização ${versaoBaixada} pronta`, body: 'Clique para instalar agora (o app reinicia).' });
    ...
    server.start(3002, () => {
        ...
        mainWindow.on('close', (e) => {
            ...
            if (!avisouBandeja && tray) {
                avisouBandeja = true;
                tray.displayBalloon({ title: 'Cordeiro Car continua rodando', content: 'O monitor de frete segue ativo aqui na bandeja. Clique duas vezes no ícone para abrir.', iconType: 'info' });
            }
        });
...
function criarBandeja() {
    ...
    tray = new Tray(icone);
    tray.setToolTip('Cordeiro Car — monitor de frete ativo');
    tray.setContextMenu(Menu.buildFromTemplate([
        { label: 'Abrir Cordeiro Car', click: () => mostrarJanela() },
        { label: 'Frete: verificar agora', click: () => {
            require('./lib/frete').verificar({ forcarTabela: true, origem: 'bandeja' }).catch(() => {});
            mostrarJanela();
            mainWindow?.loadURL('http://localhost:3002/frete');
        } },
        { label: 'Perguntas do Mercado Livre', click: () => { mostrarJanela(); mainWindow?.loadURL('http://localhost:3002/perguntas'); } },
        { type: 'separator' },
